...
export const draftService = {
  ...
  listDrafts: (status?: string): Promise<ExamDraftSummaryDTO[]> =>
    apiClient.get<ExamDraftSummaryDTO[]>(BASE, { params: status ? { status } : undefined }).then(r => r.data),
  getDraft: (draftId: string): Promise<FullDraftDTO> =>
    apiClient.get<FullDraftDTO>(`${BASE}/${draftId}`).then(r => r.data),
  editQuestion: (draftId: string, questionId: string, cmd: DraftQuestionEditCommand): Promise<void> =>
  ...
  addQuestion: (draftId: string, question: Partial<DraftQuestionDTO>, position?: number): Promise<DraftQuestionDTO> =>
    apiClient.post<DraftQuestionDTO>(`${BASE}/${draftId}/questions`, { ...question, position }).then(r => r.data),
