...
export const sessionService = {
  ...
  getQuestion: (sessionId: string, questionNumber: number): Promise<QuestionSummaryDTO> =>
    apiClient
      .get<QuestionSummaryDTO>(`${BASE}/${sessionId}/questions/${questionNumber}`)
      .then((r) => r.data),
  ...
  getReviewDashboard: (sessionId: string): Promise<ReviewDashboardDTO | null> =>
    apiClient
      .get<ReviewDashboardDTO>(`${BASE}/${sessionId}/review`)
      .then((r) => r.data)
      .catch(() => null),
  getStudentResults: (studentId: string): Promise<SessionResultSummaryDTO[]> =>
    apiClient
      .get<SessionResultSummaryDTO[]>(`${BASE}/student/${studentId}/results`)
      .then((r) => r.data),
  ...
  ): Promise<ReviewDashboardDTO> =>
    apiClient
      .patch<ReviewDashboardDTO>(`${BASE}/${sessionId}/scores/${questionId}`, payload)
      .then((r) => r.data),
};
