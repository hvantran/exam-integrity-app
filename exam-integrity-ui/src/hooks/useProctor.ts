...
export function useProctor(sessionId: string, studentId: string) {
  ...
  const report = useCallback((eventType: string) => {
    if (!sessionId) return;
  ...
  useEffect(() => {
    ...
    const onCopy   = (e: ClipboardEvent) => { e.preventDefault(); report('COPY_PASTE'); };
    const onMenu   = (e: MouseEvent) => { e.preventDefault(); report('CONTEXT_MENU'); };
