...
const rows = [
  { name: 'Kiểm tra Toán học kỳ 1 - 2024', subject: 'Toán', updated: '2 giờ trước', status: 'Đang chỉnh sửa' },
  { name: 'Đề thi Vật lý cuối kỳ', subject: 'Vật lý', updated: '1 ngày trước', status: 'Chờ duyệt' },
  { name: 'Bài kiểm tra Hóa học 15 phút', subject: 'Hóa học', updated: '3 ngày trước', status: 'Bản nháp' },
];

export const Default: Story = {
  args: {
    children: (
      <TableContainer component={Paper} elevation={0} sx={{ border: `1px solid ${colors.outlineVariant}` }}>
        <Table>
          <TableHead>
          ...
          <TableBody>
            {rows.map(row => (
              <TableRow key={row.name} hover>
                <TableCell sx={{ fontSize: '14px', color: colors.on.surface }}>{row.name}</TableCell>
